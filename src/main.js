...
const neonSelect = document.getElementById('neonSelect');
...
const CAR_SKINS = [
  { id: 'cyan', label: 'Cyan Bolt', body: '#57e7ff', glow: '#1f45b9' },
  { id: 'sunset', label: 'Sunset Drift', body: '#ff8e6a', glow: '#7c2b1c' },
  { id: 'mint', label: 'Mint Fury', body: '#88ffc6', glow: '#21634c' },
  { id: 'violet', label: 'Violet Shock', body: '#b58bff', glow: '#3a2880' },
];

const NEON_COLORS = [
  { id: 'blue', label: 'Blue Neon', color: '#7ed3ff' },
  { id: 'green', label: 'Green Neon', color: '#89ff9e' },
  { id: 'pink', label: 'Pink Neon', color: '#ff8ff4' },
  { id: 'amber', label: 'Amber Neon', color: '#ffd176' },
];
...
function updateAchievements(speed) {
  const badges = [];
  if (speed >= 280) badges.push('Speed Demon unlocked');
  if (runStats.nearMisses >= 12) badges.push('Ghost Threader unlocked');
  if (runStats.empUses >= 5) badges.push('EMP Tactician unlocked');
  if (game.wave >= 6) badges.push('Wave Breaker unlocked');
  if (game.score >= 12000) badges.push('Score Hunter unlocked');

  achievementListEl.innerHTML = '';
  if (!badges.length) badges.push('No badges yet — push harder.');
  badges.forEach((badge) => {
    const li = document.createElement('li');
    li.textContent = badge;
    achievementListEl.append(li);
  });
}
...
function populateSelectors() {
  ...
  NEON_COLORS.forEach((neon) => {
    const option = document.createElement('option');
    option.value = neon.id;
    option.textContent = neon.label;
    neonSelect.append(option);
  });
  ...
  skinSelect.value = CAR_SKINS[0].id;
  neonSelect.value = NEON_COLORS[0].id;
  zoomRange.value = '0.82';
  ...
  game.visuals.skin = CAR_SKINS.find((skin) => skin.id === skinSelect.value) || CAR_SKINS[0];
  game.visuals.neon = NEON_COLORS.find((neon) => neon.id === neonSelect.value) || NEON_COLORS[0];
  game.visuals.zoom = Number(zoomRange.value);
    repopulateParticles();
  });
  ...
  neonSelect.addEventListener('change', () => {
    game.visuals.neon = NEON_COLORS.find((neon) => neon.id === neonSelect.value) || NEON_COLORS[0];
  });
  ...
  randomChallengeBtn.addEventListener('click', () => {
    const mutators = [chaosTrafficToggle, densePickupsToggle, lowGravityToggle];
    mutators.forEach((toggle) => {
      toggle.checked = Math.random() > 0.5;
    });
    const weather = WEATHER_MODES[Math.floor(Math.random() * WEATHER_MODES.length)];
    weatherSelect.value = weather.id;
    game.weather = weather;
    repopulateParticles();
    setEventMessage('Random challenge generated.', 1.6);
    reset();
  });

  photoModeBtn.addEventListener('click', () => {
    game.visuals.photoMode = !game.visuals.photoMode;
    document.getElementById('hud').classList.toggle('photo', game.visuals.photoMode);
  });
...
function worldToScreen(x, y) {
  ...
  return {
    x: (x - player.x) * game.visuals.zoom + canvas.width / 2 + shakeX,
    y: (y - player.y) * game.visuals.zoom + canvas.height / 2 + shakeY,
  };
}
...
function render() {
  ...
  drawVehicle(player, { body: game.visuals.skin.body, glow: game.visuals.skin.glow }, 28);
  if (player.shield > 0) {
    const p = worldToScreen(player.x, player.y);
    ctx.strokeStyle = game.visuals.neon.color;
    ctx.lineWidth = 3;
    ctx.beginPath();
